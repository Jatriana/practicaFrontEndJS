...
export default class NewAnuncioFormController extends BaseController {
  ...
  añadirManejadorEventos() {
    ...
    this.elemento.addEventListener('submit', async (evento) => {
      ...
      const anuncio = {
        nombre: this.elemento.elements.nombre.value.replace(
          /(<([^>]+)>)/gi,
          ''
        ),
        precio: this.elemento.elements.precio.value.replace(
          /(<([^>]+)>)/gi,
          ''
        ),
        operacion: this.elemento.elements.operacion.value.replace(
          /(<([^>]+)>)/gi,
          ''
        ),
        descripcion: this.elemento.elements.descripcion.value.replace(
          /(<([^>]+)>)/gi,
          ''
        ),
        foto: null,
